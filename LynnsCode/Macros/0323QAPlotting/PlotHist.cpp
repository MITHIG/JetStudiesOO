...
int PlotHist(){
    ...
    string outfolder = "/home/xirong/JetStudiesOO/LynnsCode/Plots/QAPlots/0407OnePDNewest/";

    string inputFileDATA = "/home/xirong/JetStudiesOO/LynnsCode/RootFiles/0318OnePD/JetSubHist_Data_93.4MEvts_Data1PD_withjetcuts_20260319.root";
    string inputFileMCpthat15 = "/home/xirong/JetStudiesOO/LynnsCode/RootFiles/0402OnePDNewest/JetSubHist_MC_99.7MEvts_MC_1PD_xrd_Tree_test_20260402.root";
    string inputFileMCpthat0 = "/home/xirong/JetStudiesOO/LynnsCode/RootFiles/0318OnePD/JetSubHist_MC_31KEvts_pthat01PD_withjetcuts_20260319.root";
    string inputFileMCpthat15_forcuts = "/home/xirong/JetStudiesOO/LynnsCode/RootFiles/0320MultipleHistMethod/mergedPTHat15_1PD.root";
   ...
   // TNamed* generalInfoMCpthat0 = (TNamed*)fileMCpthat0->Get("GeneralInfo");
    ...
    for (size_t i = 0; i < dataHists.size() && i < mcPthat15jtHists.size(); ++i) {
      ...
      cout << "Debug: Plotting " << obsNames[i] << " with Data entries = " << (hdata ? hdata->GetEntries() : 0) 
           << " and MC entries = " << (hMC ? hMC->GetEntries() : 0) << endl;
    ...
    for (size_t i = 0; i < mcPthat15jtHists.size() && i < mcPthat15refHists.size(); ++i) {
      ...
      if (!hj || !hr || !hg){
        cout << "Debug: Missing histogram for " << obsNames[i] 
             << " (Jet: " << (hj ? "Yes" : "No") 
             << ", Ref: " << (hr ? "Yes" : "No") 
             << ", Gen: " << (hg ? "Yes" : "No") << ")" << endl;
        continue;
